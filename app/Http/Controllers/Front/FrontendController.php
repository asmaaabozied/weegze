...
//        return $currencies;
        ...
        $larages = Banner::where('type', '=', 'Large')->get();


        $top_small_bannerss = Banner::where('type', '=', 'BottomSmall')->get();


        $ps = DB::table('pagesettings')->find(1);
        $feature_products = Product::where('featured', '=', 1)->where('status', '=', 1)->select($selectable)->orderBy('id', 'desc')->latest()->take(20)->get();
        return view('front.index', compact('larages', 'ps', 'sliders', 'top_small_banners', 'feature_products', 'top_small_bannerss'));
    }
...
//        $data = geoip()->getLocation($ip);
        Visit::updateOrCreate(['ip' => $data->ip],
            ['ip' => $data->ip, 'country' => $data->countryName, 'city' => $data->cityName, 'default1' => $data->zipCode,
                'iso_code' => $data->regionCode, 'state' => $data->regionName, 'postal_code' => $data->countryCode,
                'lat' => $data->latitude, 'lon' => $data->longitude, 'currency' => $data->areaCode,'blog_id'=>$id,'type'=>'Blogs'
...
//        }
        ...
        // Redirect Section
        session()->flash('success', __('site.send_message'));
