...
//        $prods = $prods->where('status', 1)->get();
        ...
        if ($request->search) {
            ...
            $prods = Product::whereTranslationLike('name', '%' . $request->search . '%');

        }
        $prods = $prods->get();

        $prods = (new Collection(Product::filterProducts($prods)))->all();
...
//        $data = geoip()->getLocation($ip);
        Visit::updateOrCreate(['ip' => $data->ip],
            ['ip' => $data->ip, 'country' => $data->countryName, 'city' => $data->cityName, 'default1' => $data->zipCode,
                ...
                'lat' => $data->latitude, 'lon' => $data->longitude, 'currency' => $data->areaCode, 'product_id' => $productt->id, 'type' => 'Products'
