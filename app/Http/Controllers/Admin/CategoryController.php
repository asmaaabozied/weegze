    ...
    public function datatables()
    {
        ...
        return Datatables::of($datas)
            ->addColumn('status', function (Category $data) {
                ...
                return '<div class="action-list"><select class="process select droplinks ' . $class . '"><option data-val="1" value="' . route('admin-cat-status', ['id1' => $data->id, 'id2' => 1]) . '" ' . $s . '>' . trans('site.active') . '</option><option data-val="0" value="' . route('admin-cat-status', ['id1' => $data->id, 'id2' => 0]) . '" ' . $ns . '>' . trans('site.inactive') . '</option>/select></div>';
            })
            ->addColumn('attributes', function (Category $data) {
                $buttons = '<div class="action-list"><a href="' . route('admin-attr-createForCategory', $data->id) . '" class="attribute" > <i class="fas fa-edit">' . trans('site.edit') . '</i></a>';
                if ($data->attributes()->count() > 0) {
                    $buttons .= '<a href="' . route('admin-attr-manage', $data->id) . '?type=category' . '" class="edit"> <i class="fas fa-edit"></i>' . trans('site.Manage') . '</a>';
                }
                $buttons .= '</div>';
            ...
            ->addColumn('action', function (Category $data) {
                ...
                if (auth()->guard('admin')->user()->hasPermission('update_categories')) {
                    $edit = '<a  href="' . route('admin-cat-edit', $data->id) . '" class="edit" >
                            <i class="fas fa-edit"></i>
                        </a>';
                }

                if (auth()->guard('admin')->user()->hasPermission('delete_categories')) {
                    $delete = '<a href="javascript:;" data-href="' . route('admin-cat-delete', $data->id) . '" data-toggle="modal" data-target="#confirm-delete" class="delete">
