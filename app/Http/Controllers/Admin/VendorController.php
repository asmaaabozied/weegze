...
class VendorController extends Controller
{
    ...
    {
        $datas = User::where('type', 'vendor')->orderBy('id', 'desc')->get();
        ...
        return Datatables::of($datas)
            ->addColumn('status', function (User $data) {
                ...
                return '<div class="action-list"><select class="process select vendor-droplinks ' . $class . '">' .
                    '<option value="' . route('admin-vendor-st', ['id1' => $data->id, 'id2' => 2]) . '" ' . $s . '>' . trans('site.active') . '</option>' .
                    '<option value="' . route('admin-vendor-st', ['id1' => $data->id, 'id2' => 1]) . '" ' . $ns . '>' . trans('site.inactive') . '</option></select></div>';
            })
            ->addColumn('action', function (User $data) {

                $edit = '';
                $delete = '';
                if (auth()->guard('admin')->user()->hasPermission('update_vendors')) {
                    $edit = '<a href="' . route('admin-vendor-edit', $data->id) . '"> <i class="fas fa-edit"></i> ' . trans('site.edit') . '</a>';
                }

                if (auth()->guard('admin')->user()->hasPermission('delete_vendors')) {
                    $delete = '<a href="javascript:;" data-href="' . route('admin-vendor-delete', $data->id) . '" data-toggle="modal" data-target="#confirm-delete" class="delete"><i class="fas fa-trash-alt"></i> ' . trans('site.delete') . '</a>';
                }
...
//                 </div > ';
                ...
                return '<div class="godropdown"><button class="go-dropdown-toggle"> ' . trans('site.action') . '<i class="fas fa-chevron-down"></i></button><div class="action-list"><a href="' . route('admin-vendor-secret', $data->id) . '" > <i class="fas fa-user"></i> ' . trans('site.Secret Login') . '</a><a href="' . route('admin-vendor-verify', $data->id) . '" class="verify" > <i class="fas fa-question"></i> ' . trans('site.Ask For Verification') . '</a><a href="' . route('admin-vendor-show', $data->id) . '" > <i class="fas fa-eye"></i> ' . trans('site.details') . '</a><a href="javascript:;" class="send" data-email="' . $data->email . '" data-toggle="modal" data-target="#vendorform"><i class="fas fa-envelope"></i>' . trans('site.send') . '</a>' . $edit . $delete . '</div></div>';
            })
    ...
    public function subsdatatables()
    {
        ...
        return Datatables::of($datas)
            ...
            ->addColumn('action', function (UserSubscription $data) {
                return '<div class="action-list"><a href="' . route('admin-vendor-sub', $data->id) . '" class="view details-width" > <i class="fas fa-eye"></i>' . trans('site.details') . '</a></div>';
            })
            ->rawColumns(['action'])
            ->toJson(); //--- Returning Json Data To Client Side
...
//        return response()->json($msg);
        session()->flash('success', __('site.vendor-verify'));
