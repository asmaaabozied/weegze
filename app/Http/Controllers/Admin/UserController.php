...
class UserController extends Controller
{
    ...
    {
        $datas = User::where('type', 'Admin')->orderBy('id')->get();
        ...
        return Datatables::of($datas)
            ...
            ->addColumn('action', function (User $data) {
                ...
                if (auth()->guard('admin')->user()->hasPermission('update_users')) {
                    $edit = '<a  href="' . route('admin-user-edit', $data->id) . '" class="edit" >
                            <i class="fas fa-edit"></i>
                       </a>';
                }

                if (auth()->guard('admin')->user()->hasPermission('delete_users')) {
                    $delete = '<a href="javascript:;" data-href="' . route('admin-user-delete', $data->id) . '" data-toggle="modal" data-target="#confirm-delete" class="delete"><i class="fas fa-trash-alt"></i></a>';
                }
                $ban = '<select class="process select droplinks ' . $class . '">' .
                    '<option data-val="0" value="' . route('admin-user-ban', ['id1' => $data->id, 'id2' => 1]) . '" ' . $s . '>' . trans('site.block') . '</option>' .
                    '<option data-val="1" value="' . route('admin-user-ban', ['id1' => $data->id, 'id2' => 0]) . '" ' . $ns . '>' . trans('site.unblock') . '</option></select>';
                return '<div class="action-list">
<a href="' . route('admin-user-show', $data->id) . '" > <i class="fas fa-eye"></i> ' .'</a>'
                    . $edit .
 $delete . '</div>';
            })
            ->rawColumns(['action'])
    ...
    public function datatablesclients()
    {
        ...
        $datas = User::where('type', 'users')->orderBy('id')->get();
        //--- Integrating This Collection Into Datatables
        return Datatables::of($datas)
            ->addColumn('action', function (User $data) {
                $class = $data->ban == 0 ? 'drop-success' : 'drop-danger';
                ...
                $ban = '<select class="process select droplinks ' . $class . '">' .
                    ...
                    '<option data-val="1" value="' . route('admin-user-ban', ['id1' => $data->id, 'id2' => 0]) . '" ' . $ns . '>' . trans('site.unblock') . '</option></select>';
                return '<div class="action-list">
<a href="' . route('admin-user-show', $data->id) . '" > <i class="fas fa-eye"></i> ' .  '</a>' . $edit .
                    ' '. $ban . ' ' . $delete . '</div>';
            })
            ->rawColumns(['action'])
            ->toJson(); //--- Returning Json Data To Client Side
    ...
    {
        return view('admin.user.create_clients');
    ...
    public function store_clients(Request $request)
    {
        ...
        $user = User::create($request->except('photo', 'roles', 'password', 'password_confirmation') + ['type' => 'users']);
...
//        $user->update($data);
    ...
    public function store(Request $request)
    {
        $request->validate([
            ...
            'fax'   => 'required|min:11|numeric',
            'zip'   => 'required|min:0|numeric',


        ]);

        $user = User::create($request->except('photo', 'roles', 'password', 'password_confirmation') + ['type' => 'Admin']);
        $user['password'] = bcrypt($request->password);
        $user->save();
        $data = $request->all();
        if ($file = $request->file('photo')) {
...
//            'photo' => 'required',
        ...
        $data = $request->except('roles', 'password', 'password_confirmation');
