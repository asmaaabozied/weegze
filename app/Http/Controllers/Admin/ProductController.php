...
//                            })
            ...
            ->addColumn('status', function (Product $data) {
                ...
                return '<div class="action-list"><select class="process select droplinks ' . $class . '"><option data-val="1" value="' . route('admin-prod-status', ['id1' => $data->id, 'id2' => 1]) . '" ' . $s . '>' . trans('site.active') . '</option><<option data-val="0" value="' . route('admin-prod-status', ['id1' => $data->id, 'id2' => 0]) . '" ' . $ns . '>' . trans('site.inactive') . '</option>/select></div>';
            })
            ->addColumn('action', function (Product $data) {
                $catalog = $data->type == 'Physical' ? ($data->is_catalog == 1 ? '<a href="javascript:;" data-href="' . route('admin-prod-catalog', ['id1' => $data->id, 'id2' => 0]) . '" data-toggle="modal" data-target="#catalog-modal" class="delete"><i class="fas fa-trash-alt"></i> ' . trans('site.Remove Catalog') . '</a>' : '<a href="javascript:;" data-href="' . route('admin-prod-catalog', ['id1' => $data->id, 'id2' => 1]) . '" data-toggle="modal" data-target="#catalog-modal"> <i class="fas fa-plus"></i>' . trans('site.Add To Catalog') . ' </a>') : '';
                return '<div class="godropdown"><button class="go-dropdown-toggle"> ' . trans('site.action') . '<i class="fas fa-chevron-down"></i></button><div class="action-list"><a href="' . route('admin-prod-edit', $data->id) . '"> <i class="fas fa-edit"></i> ' . trans('site.edit') . '</a>
<a href="javascript:;" data-href="' . route('admin-prod-delete', $data->id) . '" data-toggle="modal" data-target="#confirm-delete" class="delete"><i class="fas fa-trash-alt"></i> ' . trans('site.delete') . '</a></div></div>';
            })
...
//            })
            ...
            ->addColumn('action', function (Product $data) {
                $catalog = $data->type == 'Physical' ? ($data->is_catalog == 1 ? '<a href="javascript:;" data-href="' . route('admin-prod-catalog', ['id1' => $data->id, 'id2' => 0]) . '" data-toggle="modal" data-target="#catalog-modal" class="delete"><i class="fas fa-trash-alt"></i>' . trans('site.Remove Catalog') . ' </a>' : '<a href="javascript:;" data-href="' . route('admin-prod-catalog', ['id1' => $data->id, 'id2' => 1]) . '" data-toggle="modal" data-target="#catalog-modal"> <i class="fas fa-plus"></i> ' . trans('site.Add To Catalog') . '</a>') : '';
                return '<div class="godropdown"><button class="go-dropdown-toggle">' . trans('site.action') . '<i class="fas fa-chevron-down"></i></button><div class="action-list"><a href="' . route('admin-prod-edit', $data->id) . '"> <i class="fas fa-edit"></i> ' . trans('site.edit') . '</a><a href="javascript" class="set-gallery" data-toggle="modal" data-target="#setgallery"><input type="hidden" value="' . $data->id . '"><i class="fas fa-eye"></i>' . trans('site.View Gallery') . '</a>' . $catalog . '<a href="' . route('admin-prod-feature', $data->id) . '" class="feature" > <i class="fas fa-star"></i> ' . trans('site.Highlight') . '</a><a href="javascript:;" data-href="' . route('admin-prod-delete', $data->id) . '" data-toggle="modal" data-target="#confirm-delete" class="delete"><i class="fas fa-trash-alt"></i> ' . trans('site.delete') . '</a></div></div>';
            })
            ->rawColumns(['name', 'status', 'action'])
            ->toJson(); //--- Returning Json Data To Client Side
    }
    ...
    public function catalogdatatables()
    {
        ...
        return Datatables::of($datas)
            ...
            ->addColumn('action', function (Product $data) {
                return '<div class="godropdown"><button class="go-dropdown-toggle"> ' . trans('site.action') . '<i class="fas fa-chevron-down"></i></button><div class="action-list"><a href="' . route('admin-prod-edit', $data->id) . '"> <i class="fas fa-edit"></i> ' . trans('site.edit') . '</a><a href="javascript" class="set-gallery" data-toggle="modal" data-target="#setgallery"><input type="hidden" value="' . $data->id . '"><i class="fas fa-eye"></i> ' . trans('site.View Gallery') . '</a><a href="' . route('admin-prod-feature', $data->id) . '" class="feature" > <i class="fas fa-star"></i> ' . trans('site.Highlight') . ' </a><a href="javascript:;" data-href="' . route('admin-prod-catalog', ['id1' => $data->id, 'id2' => 0]) . '" data-toggle="modal" data-target="#confirm-delete" class="delete"><i class="fas fa-trash-alt"></i> ' . trans('site.Remove Catalog') . '</a></div></div>';
            })
            ->rawColumns(['name', 'status', 'action'])
            ->toJson(); //--- Returning Json Data To Client Side
    }
    ...
    {
        $datas = Product::whereNotNull('discount')->orderBy('id', 'desc')->get();
    ...
    public function types()
...

        {
            ...
            return view('admin.product.create.physical', compact('cats', 'sign', 'currecny_json'));
        }
    ...
    public function createDigital()
    {
        ...
        return view('admin.product.create.digital', compact('cats', 'sign', 'currecny_json'));
    }
    ...
    public function createLicense()
    {
        ...
        return view('admin.product.create.license', compact('cats', 'sign', 'currecny_json'));
    }
...
//        return response()->json($msg);
    ...
    public function store(Request $request)
    {
        ...
        $rules = [
            ...
            'price' => 'required|same:currency.0|min:0',

            'stock' => 'nullable|min:0|numeric',
            'previous_price' => 'required|min:0|numeric',
            'discount' => '',
        ];
        if ($request->input('typetax_id') == "Fixed value") {
            $rules['discount'] = 'required|numeric|min:0|max:'.$request->price;


        }elseif($request->input('typetax_id') == "Percentage"){
...
//return $image;
            ...
            $input['slug'] = rand('111111111', '999999999');
...
//            $prod->slug = str_slug($data['name:en'],'-').'-'.strtolower(str_random(3).$data->id.str_random(3));
//        }
//        else {
//            $prod->slug = str_slug($data['name:en'],'-').'-'.strtolower($data->sku);
//        }
...
//                {
                ...
                $gallery['photo'] = $name;
                $gallery['product_id'] = Product::where('sku', $request->sku)->first()->id;
                $gallery->save();
//                }
            }
        }
        ...
        if ($request->currency) {
            ...
            foreach ($request->currency as $key => $value) {
                ...
                Currency_Product::create([
                    ...
                    'product_id' => Product::where('sku', $request->sku)->first()->id,
...
//                $input['childcategory_id'] = $request->childcategory_id;
    ...
    public function edit($id)
    {
        ...
        $currencyproduct = $data->currenies()->get();
        ...
        $currecny_json = $currency->map(function ($items, $parentkey) use ($baseChange) {
            ...
            return view('admin.product.edit.physical', compact('cats', 'data', 'sign', 'currecny_json', 'currencyproduct'));
    }
...
//return  $request;
        ...
        if ($request->currency) {
            Currency_Product::where('product_id', $id)->delete();

            foreach ($request->currency as $key => $value) {

//                $data->currenies()->delete();
...
//            }
        ...
        if ($data->type == "License") {
            if (!empty($request->license == null)) {

//            if (!in_array(null, $request->license) && !in_array(null, $request->license_qty)) {
//                $input['license'] = implode(',,', $request->license);
                $input['license'] = $request->license ?? '';
                $input['license_qty'] = $request->license_qty ?? '';
