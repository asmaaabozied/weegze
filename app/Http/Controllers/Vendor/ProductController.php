...
//                <a href="javascript" class="set-gallery" data-toggle="modal" data-target="#setgallery"><input type="hidden" value="' . $data->id . '"><i class="fas fa-eye"></i> ' . trans('site.view') . '</a>
                return '<div class="action-list"><a href="' . route('vendor-prod-edit', $data->id) . '"> <i class="fas fa-edit"></i>' . trans('site.edit') . '</a><a href="javascript:;" data-href="' . route('admin-prod-delete', $data->id) . '" data-toggle="modal" data-target="#confirm-delete" class="delete"><i class="fas fa-trash-alt"></i></a></div>';
            })
...
//            })
            ...
            ->addColumn('action', function (Product $data) {
                ...
                $catalog = $ck ? '<a href="javascript:;"> Added To Catalog</a>' : '<a href="' . route('vendor-prod-catalog-edit', $data->id) . '"><i class="fas fa-plus"></i> ' . trans('site.edit') . '</a>';
    ...
    public function createPhysical()
    {
        ...
        return view('vendor.product.create.physical', compact('cats', 'sign','currecny_json'));
    }
    ...
    public function createDigital()
    {
        ...
        return view('vendor.product.create.digital', compact('cats', 'sign','currecny_json'));
    }
    ...
    public function createLicense()
    {
        ...
        return view('vendor.product.create.license', compact('cats', 'sign','currecny_json'));
    }
...
//                $input['childcategory_id'] = $request->childcategory_id;
    ...
    public function store(Request $request)
    {
        ...
        $data['details:en']=$request['details:en'];

        $data['details:ar']=$request['details:ar'];

        $data['policy:en']=$request['policy:en'];
        $data['is_catalog']=1;

        $data['policy:ar']=$request['policy:ar'];
...
//        $data['type_check']=$request['type_check'];
...
//        $data['product_type']=$request['type'];
        ...
        $data['price_dollars']=$request['price_dollars'];

        $data['price_egyptian']=$request['price_egyptian'];

        $data['price_riyals']=$request['price_riyals'];

        $data['price_sudanese']=$request['price_sudanese'];

        $data['status']=$request['status'];
        $data['user_id']=\Illuminate\Support\Facades\Auth::id();
        $data->save();

//        $input = $request->except('photo','features','colors','file','_token');


        // Save Data
//        Product::create($input + ['user_id', \Illuminate\Support\Facades\Auth::id()])->save();
...
//    }
    ...
    public function edit($id)
    {
        ...
        if ($data->type == 'Digital')
            return view('vendor.product.edit.digital', compact('cats', 'data', 'sign','currecny_json'));
        elseif ($data->type == 'License')
            return view('vendor.product.edit.license', compact('cats', 'data', 'sign','currecny_json'));
        else
            return view('vendor.product.edit.physical', compact('cats', 'data', 'sign','currecny_json'));
    }
