									...
									{{ Session::has('cart') ? count(Session::get('cart')->items) : '0' }}
														</span> @lang('site.items')
													</span>

													<a class="view-cart" href="{{ route('frontendd.cart') }}">
													@lang('site.cart')
													</a>
												</div><!-- End .dropdown-cart-header -->
												<ul class="dropdown-cart-products">
													@foreach(Session::get('cart')->items as $product)
														<li class="product creclass="product-image"move{{ $product['item']['id'].$product['size'].$product['color'].str_replace(str_split(' ,'),'',$product['values']) }}">
												...
												<div class="dropdown-cart-action">
														<a href="{{ route('frontend.checkout') }}" class="mybtn1">{{trans('site.checkout')}}</a>
												</div><!-- End .dropdown-cart-total -->
										</div>
									@else
@lang('site.emptycart')
