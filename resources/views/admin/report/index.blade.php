...
@section('content')
					...
					<div class="content-area">
						<div class="mr-breadcrumb">
							<div class="row">
								<div class="col-lg-12">
										<h4 class="heading">{{ __('site.Reports') }}</h4>
										<ul class="links">
											<li>
											...
											<li>
												<a href="{{ route('admin-report-index') }}">{{ __('site.Reports') }}</a>
											</li>
										</ul>
								</div>
							</div>
								...
								<div class="heading-area">
	                                ...
	                                <div class="action-list">
	                                    <select class="process select droplinks {{ $gs->is_report == 1 ? 'drop-success' : 'drop-danger' }}">
	                                      <option data-val="1" value="{{route('admin-gs-isreport',1)}}" {{ $gs->is_report == 1 ? 'selected' : '' }}>{{ __('site.active') }}</option>
	                                      <option data-val="0" value="{{route('admin-gs-isreport',0)}}" {{ $gs->is_report == 0 ? 'selected' : '' }}>{{ __('site.inactive') }}</option>
	                                    </select>
	                                  </div>
								</div>
									<div class="mr-table allproduct">
										...
										<div class="table-responsiv">
												<table id="geniustable" class="table table-hover dt-responsive" cellspacing="0" width="100%">
													<thead>
														<tr>
									                        <th width="15%">{{ __('site.products') }}</th>
									                        <th width="15%">{{ __('site.Reporter') }}</th>
									                        <th width="30%">{{ __('site.title1') }}</th>
									                        <th>{{ __('site.date') }}</th>
