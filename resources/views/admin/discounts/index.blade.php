...
@section('content')
    ...
    <div class="content-area">
        <div class="mr-breadcrumb">
            <div class="row">
                <div class="col-lg-12">
                    <h4 class="heading">{{ __('site.discountss') }}</h4>
                    <ul class="links">
                        <li>
                            <a href="{{ route('admin.dashboard') }}">{{ __('site.dashboard') }} </a>
                        </li>
                        <li>
                            <a href="javascript:;">{{ __('site.discountss') }} </a>
                        </li>
...
{{--                              <a  onclick="window.location='?id={{$product->id}}';">{{$product->name}}</a>--}}
{{--                                @endforeach--}}
                    ...
                    <div class="mr-table allproduct">
                        ...
                        <div class="table-responsiv">
                            <table id="geniustable" class="table table-hover dt-responsive" cellspacing="0"
                                ...
                                <tr>
                                    <th>{{ __('site.name') }}</th>
                                    <th width="40%">{{ __('site.start_at') }}</th>
                                    <th>{{ __('site.end_at') }}</th>
                                    <th>{{ __('site.type') }}</th>
...
@section('scripts')
    ...
    <script type="text/javascript">
        ...
        var table = $('#geniustable').DataTable({
            ...
            processing: true,
            serverSide: true,
            ajax: '{{ route('admin-discounts-datatables') }}',
            columns: [
                {data: 'name', name: 'name'},
        ...
        $(function () {
            ...
            @if(auth()->guard('admin')->user()->hasPermission('create_discountss'))

            $(".btn-area").append('<div class="col-sm-4 table-contents">' +
                '<a class="add-btn" href="{{route('admin-creatediscounts-index')}}">' +
