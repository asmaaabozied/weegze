            ...
            <div class="row">
                <div class="col-lg-12">
                    ...
                    <ul class="links">
                        ...
                        <li>
                            <a href="{{ route('admin-currency-index') }}">{{ __('site.Currencies') }}</a>
                        </li>
                    </ul>
                </div>
            </div>
                    ...
                    <div class="heading-area">
                        ...
                        <div class="action-list">
                            <select
                                ...
                                <option data-val="1"
                                        value="{{route('admin-gs-iscurrency',1)}}" {{ $gs->is_currency == 1 ? 'selected' : '' }}>{{ __('site.active') }}</option>
                                <option data-val="0"
                                        value="{{route('admin-gs-iscurrency',0)}}" {{ $gs->is_currency == 0 ? 'selected' : '' }}>{{ __('site.deactive') }}</option>
                            </select>
                        </div>
                    </div>
...
@section('scripts')
    ...
    <script type="text/javascript">
        ...
        $(function () {
            ...
            @if(auth()->guard('admin')->user()->hasPermission('create_currencies'))

            $(".btn-area").append('<div class="col-sm-4 text-right">' +
                '<a class="add-btn" href="{{route('admin-currency-create')}}" id="add-data" >' +
