...
<div class="mr-breadcrumb">
    <div class="row">
        <div class="col-lg-12">
            ...
            <h4 class="heading">{{ __('site.number') }}: {{$conv->order_number}}</h4>
            @endif
            <h4 class="heading">{{ __('Conversation with') }} {{$conv->user->name}} <a class="add-btn" href="{{ url()->previous() }}"><i class="fas fa-arrow-left"></i> {{ __('site.back') }}</a></h4>
                <ul class="links">
                    <li>
                        <a href="{{ route('admin.dashboard') }}">{{ __('site.dashboard') }} </a>
                    </li>
                    <li>
                        <a href="{{ route('admin-message-index') }}">{{ __('site.message') }}</a>
                    </li>
                    <li>
...
<div class="order-table-wrap support-ticket-wrapper ">
                        ...
                        @include('includes.admin.form-both')
                            ...
                            <div class="panel-footer">
                                <form  action="{{route('admin-message-store')}}" data-href="{{ route('admin-message-load',$conv->id) }}" method="POST">
                                    {{csrf_field()}}
